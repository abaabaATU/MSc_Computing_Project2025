...
from .utils import demean, isbad
...
def fit_var1(data, mode="OLS", detrend=True):
    """
    ...
    if mode == "OLS":
        ...
        X_t  = data[:,:-1,:].reshape(n, M)  # All timepoints except the last (past)
        X_tp1 = data[:,1:,:].reshape(n, M)  # All timepoints except the first (future)
...
def fit_var(data, p=None, maxp=None, mode='OLS', detrend=True):
    ...
    if p is None:
        ...
        V = np.cov(data.reshape(2, -1))  # (2, 2) covariance matrix
        A = np.zeros((2, 2, 1))  # 零系数矩阵
        return A, V, 0, None  # 返回伪VAR(0)结果
    else:
