import numpy as np
from .VAR_NuMIT import *
from .Gaussian_NuMIT import *
from .utils import CompQuantile, arrayfy
import warnings
...
class NuMIT: 
    ...
    def normalise(self, As=None, corr=True, parallel=False, n_jobs=4, add_measures=None, **kwargs):  
        """
        ...
        Parameters:
            ...
            add_measures (list of str):             Additional measurements to normalise. Only valid if self.kind ="PhiID".
        ...
        if self.params["model"]=="VAR":  
            ...
            self.nulls = Gauss_nulls(self.MIs, self.kind, self.params, self.verbose)
        else: 
        ...
        if self.kind=="PhiID" and add_measures is not None:
            self.normalise_add_measures(add_measures)



    def normalise_add_measures(self, add_measures=None):
        """
        Normalise additional measures.

        Parameters:
            add_measures (list of str): List of additional measures to normalize.
        """
        if self.kind=="PhiID" and add_measures is not None:
            # do some input checks
            self.valid_measures = {"TExy", "TEyx", "AISx", "AISy", "PhiWMS", "PhiR", "CD"}
            add_measures = self.check_add_measures(add_measures)
            if add_measures is not None:
                self.add_measures = {}
                self.nulls_add_measures = {}
                self.normalised_add_measures = {}

                for meas in add_measures:
                    # compute the measures and their nulls
                    self.compute_info(meas)
    ...
    def compute_info(self, meas):
        """
        Compute the information measures for the given atoms and null models.

        Parameters:
            meas (str): The tbpe of information measure to compute.

        Returns:
            numpy.ndarray: Information measures computed from the atoms and nulls.
        """
        assert meas in self.valid_measures, f"Additional information measure not supported!"
        labels = ["rtr", "rta", "rtb", "rts", "xtr", "xta", "xtb", "xts", "ytr", "yta", "ytb", "yts", "str", "sta", "stb", "sts"]
        get_indices = lambda labs, labels: [labels.index(lab) for lab in labs]

        # exchange
        if meas=="Exchange":
            labs = ["xtb", "yta"]
        elif meas=="TE":
            labs = ["xtr", "ytr", "xtb", "yta", "str", "sta"],
        elif meas=="TExy":
            labs = ["xtr", "xtb", "str", "stb"]
        elif meas=="TEyx":
            labs = ["ytr", "yta", "str", "sta"]

        # storage and memory
        elif meas=="Storage":
            labs = ["rtr", "xta", "ytb", "sts"]
        elif meas=="AIS":
            labs = ["rtr", "rta", "rtb", "xtr", "xta", "ytr", "ytb"]
        elif meas=="AISx":
            labs = ["rtr", "rta", "xtr", "xta"]
        elif meas=="AISy":
            labs = ["rtr", "rtb", "ytr", "ytb"]
        elif meas=="Memory":
            labs = ["xta", "ytb"]
        elif meas=="Memory_x":
            labs = ["xta"]
        ...
        # integration 
            ...
            labs = ["rts", "xts", "yts", "str", "sta", "stb"]
        elif meas=="PhiWMS" or meas=="PhiR":
            labs = ["rts", "xtb", "xts", "yta", "yts", "str", "sta", "stb", "sts"]

        # extra
        elif meas=="CD":
            labs = ["xtr", "xtb", "ytr", "yta", "str", "str", "sta", "stb"]
        elif meas=="CDnorm":
            labs = ["xtr", "xtb", "ytr", "yta", "str", "sta", "stb"]

        indx = get_indices(labs, labels)
        self.add_measures[meas] = np.sum(self.atoms[indx,:], axis=0)
        self.nulls_add_measures[meas] = np.sum(self.nulls[indx,:,:], axis=0)

        # remove rtr atom for PhiWMS
        if meas=="PhiWMS":
            self.add_measures[meas] = self.add_measures[meas] - self.atoms[0,:]
            self.nulls_add_measures[meas] = self.nulls_add_measures[meas] - self.nulls[0,:,:]
